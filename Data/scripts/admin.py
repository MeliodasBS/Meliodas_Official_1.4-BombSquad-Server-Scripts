...
class PermissionEffect(object):
    def __init__(self, position=(1,0, 0), owner=None, prefix='ADMIN', prefixColor=(1, 1, 1),
        ...
        # prefix
        if type == 1:
            m = bs.newNode('math', owner=self.owner, attrs={'input1': (0, 1.94, 0), 'operation': 'add'})
        else:
            m = bs.newNode('math', owner=self.owner, attrs={'input1': (0, 1.64, 0), 'operation': 'add'})
        ...
        bs.animate(self._Text, 'scale', {0: 0.0, 1200: 0.01})  # smooth prefix spawn
...
class Enhancement(bs.Actor):
	...
	def getTag(*args):
		...
		if cl_str in gph.customlist:
			    ...
			    PermissionEffect(owner = spaz.node,prefix = tag,prefixAnim = {0: (1,0,0), 250: (0,1,0),250*2:(0,0,1),250*3:(1,0,0)})
                elif cl_str in gph.coownerHashes:
			    tag = getTag(1)
			    if tag == '0': tag = u'\ue043CO-OWNER\ue043'
			    PermissionEffect(owner = spaz.node,prefix = tag,prefixAnim = {0: (1,0,0), 250: (0,1,0),250*2:(0,0,1),250*3:(1,0,0)})
                elif cl_str in gph.adminHashes:
			    tag = getTag(1)
			    if tag == '0': tag = u'\ue043ADMIN\ue043'
			    PermissionEffect(owner = spaz.node,prefix = tag,prefixAnim = {0: (1,0,0), 250: (0,1,0),250*2:(0,0,1),250*3:(1,0,0)})
	...
	if settings.enableStats:
		...
		if aid in pats:
			...
			display = '🌿[' + str(rank) + ']'
		        PermissionEffect(owner = spaz.node,prefix = display,prefixAnim = {0: (0, 1, 0)},type=2)
		    else:
			display = '🌿[' + str(rank) + ']'
