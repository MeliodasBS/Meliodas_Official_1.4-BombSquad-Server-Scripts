...
class Session(object):
	...
	ID = player.get_account_id()
	if ID in gph.banlist:
		clientID =player.getInputDevice().getClientID()
		#find name
		#name = player.getName()
		name = gph.banlist[ID]
		bsInternal._chatMessage("%s, lovely tera papa hai aur papa ke server me allowed nahi tu nikal sale"%name)
	...
	for i in bsInternal._getGameRoster():
		try:
			if i['clientID']==clientID:
				displayString = i['displayString'].decode('utf8')
		except:
			pass
	import json
	logFile = 'logPlayers.json'
	from os import path
	...
	if ID not in allPlayers:
		allPlayers[ID] = [displayString]		
	elif displayString not in allPlayers[ID]:
		allPlayers[ID].append(displayString)
	...
	with io.open(logFile,'w',encoding='utf8') as f:
		...
		f.write(json.dumps(allPlayers, ensure_ascii=False, indent=4, sort_keys=True))"""
    ...
    def _requestPlayer(self, player):
        ...
        if result:
			...
			global lastJoiner
			try:
			    #print 'checking %s	%s'%(lastJoiner,ID)
			    if ID == lastJoiner:
				#print 'possible spammer case'
				with open('possibleSpammer.txt','a') as f:
					f.write(ID+'\n')
			    lastJoiner = ID
			except:
